		...
		with tf.Session() as sess:
			...
			for e in range(self.n_epochs):
				...
				for i in range(len(x)):
					if self.cpt_ext:
						...
						feed_dict = {self.model.x: np.array(x[i]), self.model.y: np.array(y[i]), self.model.is_training: True}
				...
				for i in range(len(y_val)):
					if self.cpt_ext:
						...
						feed_dict = {self.model.x: x_val[i], self.model.y: y_val[i], self.model.is_training: False}
			...
			for i in range(len(y_test)):
				if self.cpt_ext:
					...
					feed_dict = {self.model.x: x_test[i], self.model.y: y_test[i], self.model.is_training: False}
