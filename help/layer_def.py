...
def conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False, reuseL=True):
  with tf.variable_scope('{0}_conv'.format(idx),reuse=reuseL) as scope:
...
def transpose_conv_layer(inputs, kernel_size, stride, num_features, idx, linear = False, reuseL=True):
  with tf.variable_scope('{0}_trans_conv'.format(idx),reuse=reuseL) as scope:
    #input_channels = inputs.get_shape()[3]
    ...
    #output_shape = tf.stack([batch_size, input_shape[1]*stride, input_shape[2]*stride, num_features])
...
def fc_layer(inputs, hiddens, idx, flat = False, linear = False, reuseL=True):
  with tf.variable_scope('{0}_fc'.format(idx), reuse=reuseL) as scope:
    input_shape = inputs.get_shape().as_list()
    if flat:
